...
func TestReadProjectsMaxDepth(t *testing.T) {
	...
	cleanup := buildFakeDomainsDirectory(t, nGroups, levels, true)
	defer cleanup()
...
func buildFakeDomainsDirectory(t require.TestingT, nGroups, levels int, verbose bool) func() {
...
func benchmarkReadGroups(b *testing.B, groups, levels int) {
	cleanup := buildFakeDomainsDirectory(b, groups, levels, false)
	defer cleanup()
...
func BenchmarkReadGroups(b *testing.B) {
	...
	b.Run("10000 groups 1 levels", func(b *testing.B) { benchmarkReadGroups(b, 1000, 1) })
}
