...
var (
	...
	reqDomainPerSecond      = flag.Duration("req-domain-per-second", ratelimiter.DefaultRatePerDomainPerSecond, "Requests per domain limit per second")
	reqDomainBucketSize     = flag.Int("req-domain-bucket-size", ratelimiter.DefaultPerDomainMaxBurstPerSecond, "Bucket size = max number of tokens held by the limiter")
