...
function push_tags(){
  ...
  # If a version has been specified and this is not a regular branch,
  # we use the specified version.
  if [ -n "$1" ] && (is_master || is_tag); then
    local edition=$1

    # If on a tag pipeline, we can trim the `-ee` and `-ubi8` suffixes.
    if is_tag; then
    ...
    # If no version is specified, but on a tag pipeline, we use the trimmed tag.
    trimmed_tag=$(trim_edition $CI_COMMIT_TAG)

    tag_and_push $trimmed_tag
  else
    # If no version is specified, but on a branch, we use the branch slug.
